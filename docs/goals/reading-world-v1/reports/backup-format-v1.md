...
恢复提供“空库副本”和“合并当前书架”两种模式。空库副本只允许目标书架为空；合并模式先生成只读计划并列出同 ID 内容冲突，不得以覆盖现有书架代替合并。
...
- 新 ID 的书、章、书签与文件引用直接加入。
- 公开契约归一化后内容一致的同 ID 条目跳过，不因字段插入顺序或数据库私有扩展字段制造冲突。
- 同 ID 且公开内容不同的书、章、书签、文件引用与阅读设置必须逐项选择“保留现有”或“使用备份”；未解决冲突时计划不可执行且零写入。
- 阅读进度按 `updatedAt` 只向更新记录前进，不回退到较旧进度。
- 执行前保存当前完整快照；写后完整回读。写入或回读失败时恢复旧快照并再次验证，回滚失败同时报告原始错误与补偿错误。

## 人读导出

书签与笔记可另行下载稳定 UTF-8 JSON 或 Markdown。导出仅含书名、章节号、位置、摘录、笔记与创建时间；不含本机路径、目录句柄、设备标识或凭证。Markdown 对结构字符做转义并把跨行摘录逐行引用；JSON 使用公开的 `read-realm-human-notes` v1 信封和稳定排序。
