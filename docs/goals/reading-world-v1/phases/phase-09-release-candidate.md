...
- ACT-08 部署检查点：只有本阶段候选、本地门禁与人工体验通过后，才可连接 `root@tx.guyue.site`。先只读盘点宝塔管理的现有站点、监听端口、反向代理、证书和磁盘；部署必须使用独立目录与内部端口，不停止、不覆盖、不改写既有服务。若需新增公网端口、变更既有站点/证书或迁移真实数据，立即暂停并请求确认。
