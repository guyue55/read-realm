...
- 当前控制基线：提交 c900af34e81d5b09319498f57953bf2c0205c02c 与方向授权摘要 SHA-256 3296525f366ebb43b0df89b2bf0b44de2a48d90bd908bafc5d0bc08f52ca7b4a。
- 替代或继承：无旧控制包；历史规格和 2026-07-11 验收报告仅作调查线索。
...
|---|---|---|---|---|---|---|
| REV-0001 | 无（首次建立） | c900af34e81d5b09319498f57953bf2c0205c02c + 方向摘要 3296525f | 无（首次建立） | 无（首次建立） | ACTIVE | 用户逐项关闭范围、数据、同步、体验、验收和授权边界后首次建立 |
...
| HYP-01 | HYPOTHESIS | 现有核心架构可通过演进式提取与修复达到目标，不需全仓重写 | 已有共享包、领域目录和可运行主旅程 | EXP-01/02/03 任一先通过早期薄切片 GATE-01；三次均失败则熔断 | EXP-01、EXP-02、EXP-03 | 每次记录独立 ATTEMPT；三次失败进入设计复核，不临时追加第四次 |
...
## 风险门与先纵切后扩张
- 扩张规则：先纵切后扩张；PHASE-02 必须用现有 UI 和最小兼容层完整通过早期薄切片 GATE-01，才能进入 PHASE-03 并批量扩张导入、阅读、书架、PWA 或迁移；GATE-02 通过前不得把全文同步扩到正式入口。
- 设计复核规则：同一风险门在 REV-0001 下完成 3 次差异化实验仍失败，状态写入 BLOCKED_DESIGN_REVIEW_REQUIRED，停止堆实现并请求新修订。
...
## 循环与预算
- 最大修复轮数：每项任务 3 轮差异化修复；同一风险门 3 次差异化实验失败立即设计复核。
