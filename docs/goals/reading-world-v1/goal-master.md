...
- 最后审查时间：2026-08-14T00:27:00+08:00
...
- 当前控制基线：实现提交 66f2ba500d8c11ddcdfbee4e1414b0c3bf92556a；REV-0001 方向摘要 SHA-256 3296525f366ebb43b0df89b2bf0b44de2a48d90bd908bafc5d0bc08f52ca7b4a；REV-0002 用户批准摘要 SHA-256 9858a1c86f33c74aa42ce5801060d1dca7b974881c0e140a56e7e9af5de1a7df；REV-0003 用户批准摘要 SHA-256 e88b58dea87dd1f409190b0b75759df08f7e9eafe8946763c77841124234096c。
- 替代或继承：REV-0003 继承 REV-0002 的全部实现、阶段结论、完成定义、GATE-00/01/02 与历史 ATTEMPT，只替代“完全不建设内容平台”的范围决定，增加单实例共享馆藏、GATE-03 与 PHASE-05 交付；公共馆藏明文域不得替代 PHASE-07 私有同步密文域。EVID-27/28/29 和所有历史失败永久保留。历史规格和 2026-07-11 验收报告仍仅作调查线索。
...
- 本 Goal 交付：在现有 Monorepo 内交付生产级候选的本地优先 Web/PWA 阅读系统，完整覆盖正式核心旅程、数据可携带性、单实例共享藏经阁、端到端加密自托管私有同步和跨环境验收。
- 时间型结果：数周至数月真实阅读后的长期舒适度、长期同步可靠性、NAS/VPS 运维稳定性与用户习惯留存；本 Goal 不提前宣称这些结果完成。
- 活跃控制文档上限：12
- 控制包推翻条件：若真实回放证明 REV-0003 无法在不丢数据、不破坏离线核心、不混淆公共明文/私有密文或不引入不可接受复杂度下实现，立即暂停受影响范围并请求用户批准新控制修订，禁止静默降标。
...
| REV-0002 | REV-0001 | fbbbf5642ee8e4aa36699d8f466fc630f0f93447 + 批准摘要 9858a1c86f33c74a | ACT-07 | GATE-01 | SUPERSEDED | 用户批准设计复核恢复：先证明验证基础设施资格，再注册产品复验；旧 ATTEMPT 不删除、不改写、不转为通过 |
| REV-0003 | REV-0002 | 66f2ba500d8c11ddcdfbee4e1414b0c3bf92556a + 批准摘要 e88b58dea87dd1f4 | ACT-09 | 无（主动修订） | ACTIVE | 用户批准在当前版本增加单实例共享藏经阁；公共明文域与私有同步密文域严格分离，先纵切后扩张 |
...
| FDEC-01 | FROZEN_DECISION | 本地优先、Web/PWA、可选自托管私有云，不建设账号/多用户社交平台或原生 App；REV-0003 仅增加单实例共享馆藏 | direction-approval-v1.md、rev-0003-approval.md | 只有用户批准的新 REV 可改变 | FACT-01 | 暂停受影响范围并追加控制修订 |
...
| FACT-05 | VERIFIED_FACT | 现有非默认分享密钥只提供服务端数据作用域和请求凭据，不是设备端加密密钥；当前书籍/章节 API 保存明文 | book/folder/search repository、share-token 装饰器与定向源码审计 | 后续实现证明密钥材料只在设备端且服务端只见密文 | FDEC-04 | 禁止把分享密钥宣传为 E2EE 或复用私有同步事实源 |
| FDEC-04 | FROZEN_DECISION | REV-0003 增加单实例共享藏经阁：公共内容服务端明文、访客只读、非默认分享密钥作上传维护凭据；不建设账号/社交/治理平台 | rev-0003-approval.md | 只有用户批准的新 REV 可改变；公共域与私有同步域不得合并 | FACT-05 | 暂停藏经阁并追加控制修订 |
| HYP-05 | HYPOTHESIS | 单实例公共馆藏可在不阻断本地书架/阅读、不混淆私有同步的前提下完成发布、浏览、加入本地与离线阅读 | 现有 API/SQLite/Blob、分享密钥作用域、PHASE-03 导入/备份与 PHASE-04 ReaderSession | EXP-14/15/16 任一完整通过 GATE-03；三次均失败则熔断 | EXP-14、EXP-15、EXP-16 | 保持藏经阁隐藏，保留 ATTEMPT，进入设计复核 |
| EXP-14 | EXPERIMENT | 基线机制：非默认分享密钥直接上传一份固定合法 TXT，匿名浏览公共目录，加入本地书架后真断网阅读 | clean PHASE-04/05 候选、固定 TXT、隔离公共库 | 完整通过 GATE-03；失败写 EVID-59 | HYP-05 | 保留 ATTEMPT，转 EXP-15 的维护目录关联机制 |
| EXP-15 | EXPERIMENT | 存储差异：服务端配置固定维护目录，深度扫描并关联原文件，再完成相同匿名浏览、加入本地与离线阅读结果 | EXP-14 ATTEMPT、固定目录树 | 完整通过 GATE-03；失败写 EVID-60 | HYP-05 | 保留 ATTEMPT，转 EXP-16 的个人书架发布机制 |
| EXP-16 | EXPERIMENT | 入口差异：从已有云端正文的个人书架发布到公共域，再完成相同匿名浏览、加入本地与离线阅读结果 | 前两次 ATTEMPT、固定已上传书 | 完整通过 GATE-03；失败写 EVID-61，第三次失败即熔断 | HYP-05 | 写入 BLOCKED_DESIGN_REVIEW_REQUIRED 并请求新 REV |

## 风险门与先纵切后扩张
- 扩张规则：先纵切后扩张；REV-0002 已通过 GATE-00/01 并放行 PHASE-03~06；GATE-02 通过前不得把全文私有同步扩到正式入口。REV-0003 的藏经阁必须在 PHASE-05 先通过 GATE-03，才允许扩张服务端目录扫描、多视图、批量上传或个人书架发布。
...
| GATE-03 | 用户可从单实例共享馆藏取得一本公共书并保持本地优先阅读 | 持非默认分享密钥发布固定合法书 -> 匿名分类/检索/分页浏览 -> 加入本地书架 -> 真断网打开正文 -> 公共服务故障不影响已加入书 | 默认/缺失密钥可写、公共与私有表混用、密钥泄露、未完整复制即伪成功、断网不可读或公共 API 故障阻断本地阅读 | EVID-57 | 允许在 PHASE-05 扩张目录扫描、分类标签模板、多视图和批量入口 |
...
- 已关闭冲突：REV-0003 允许单实例共享馆藏，但继续放弃账号/多用户社交与治理平台、原生 App、官方托管云、内置侵权书源和 AI 核心化。
- 事实源：代码与运行产物为实现事实源；IndexedDB/OPFS 是个人本地事实源；公共馆藏 SQLite/Blob 是公开明文事实源；PHASE-07 私有同步 SQLite/Blob 只保存密文与设备状态，二者不得共表或共用“加密”文案；后端执行访问控制，前端只表达状态。
- 当前基线：旧主旅程和 PWA 有候选实现与旧绿灯，但完整备份、版本迁移、端到端同步、恢复码/设备撤销、Docker 交付和本 Goal 压力/跨环境证据均未证明。

## 范围契约
- 必须完成：稳定书架、TXT/EPUB/文件夹/合法 URL 导入、解析预览、沉浸阅读与准确续读、搜索、书签笔记、完整备份恢复与 Markdown/JSON 导出、单实例共享藏经阁、生产离线 PWA、端到端加密自托管私有同步、清晰同步 UI、跨浏览器与压力验收。
- 明确不做：账号/多用户社交平台、公共馆藏审核/版权投诉/下架流程/配额/恶意文件隔离/完整审计、商城、社交、广告、推荐流、原生 App、官方托管云、内置盗版书源、绕过登录/付费/反爬、AI 核心依赖、PostgreSQL/Redis/Kubernetes。
- 必须保持：现有用户数据、旧稳定版本可读性、安静柔和纸感国风基调、核心功能无 AI/无网络仍可用、服务端不见私有同步明文；公共馆藏明文必须独立标识和存储。
...
### 推荐方案
- 总体方案：以 GATE-01 本地纵向切片先证明数据与阅读核心；PHASE-05 用 GATE-03 独立证明公共馆藏；GATE-02 独立证明端到端私有同步后才正式开放。
- 架构边界：页面只组合 UI；领域服务管理书架、导入、阅读、备份、公共馆藏和私有同步；reader/parser/storage 核心保持无 UI；浏览器与 API 存储走适配器；公共馆藏服务保存明确公开的明文，私有同步服务只保存密文与设备授权状态。
...
| PHASE-05 | 收敛书架/辅助 UI 并交付单实例共享藏经阁 | PHASE-04 | 统一领域入口、共享反馈组件、GATE-03、公共馆藏浏览/加入/上传/扫描 | PHASE-04 | 组件/领域/API/E2E | 500 本书架、公共馆藏纵切与单手操作 | UI 保持品牌气质、公共/私有语义清楚 |
...
| DEC-15 | 发布候选与人工体验通过后，可在 `root@tx.guyue.site` 以独立目录/内部端口部署；先只读盘点宝塔与既有服务，不覆盖、不停止、不改写现有站点/代理/证书 | PHASE-09 | EVID-54 |
| DEC-16 | REV-0003 采用单实例共享藏经阁：公共内容服务端明文、访客只读、非默认分享密钥仅作上传维护凭据，严格区别私有同步密钥 | PHASE-05 | EVID-55 |
...
| REQ-16 | 藏经阁支持公共书浏览/检索/分页/分类标签/维护者视图、加入本地、凭据上传、书架发布和维护目录深度扫描关联 | PHASE-05 | EVID-56 |
| RISK-01 | 先通过本地纵向切片再扩张，不允许数据丢失或伪成功 | PHASE-02 | EVID-17 |
| RISK-02 | 端到端同步服务端无明文且冲突、撤销、恢复均可验证 | PHASE-07 | EVID-18 |
| NREQ-01 | 不出现商城、社交、广告、推荐流、账号/多用户社交平台或原生 App 范围膨胀；REV-0003 单实例共享馆藏不视为账号平台 | PHASE-09 | EVID-19 |
...
| NREQ-07 | 不把分享密钥冒充账号或 E2EE 密钥，不把公共明文混入私有同步密文域，不宣称具备审核/版权/下架/恶意文件治理 | PHASE-08 | EVID-58 |
| RISK-03 | 迁移和协议变化必须先备份、幂等、失败回滚且至少读上一稳定版 | PHASE-02 | EVID-25 |
| RISK-04 | GATE-01 验证设施必须先证明可判定、无遗留副作用且证据可复算 | PHASE-02 | EVID-45 |
| RISK-05 | 藏经阁先证明发布到离线阅读的公共纵切，且不混用私有同步事实源、不阻断本地核心 | PHASE-05 | EVID-57 |
...
## 最终完成定义
- 正向条件：全部阶段为完成；REQ、DEC、RISK 的 FINAL 证据新鲜通过；GATE-01/GATE-02/GATE-03 通过；正式核心旅程、公共馆藏、备份恢复、PWA 离线、密文私有同步、Docker、性能和浏览器矩阵均有活体证据。
- 否定清单：任何 NREQ 出现、任何数据丢失/伪成功、任何分享密钥泄漏或私有同步明文、任何公共/私有域混淆、任何目标环境未验证、任何旧证据冒充新鲜证据、任何公开部署/付费/删数据越权，均不得完成。
...
| ACT-02 | 公开部署、发布镜像、付费或发送外部消息 | 待授权 | 每次具体动作、环境或参数变化均需新授权 | 是 | PENDING | 待授权 |
...
| ACT-08 | 所有本地发布门通过后，在 `root@tx.guyue.site` 部署一份；先只读盘点宝塔、端口、现有站点与反代，再使用独立目录、独立内部端口和不覆盖既有服务的反向代理 | EVID-54；目标主机与“不影响既有宝塔服务”约束明确 | 本地发布门未通过、需覆盖/停用既有服务、需开放额外公网端口、域名/证书/部署目录不明确、涉及真实数据迁移或破坏性动作时暂停并重新确认 | 是 | APPROVED | DEC-15 |
| ACT-09 | 创建并执行 REV-0003：在 PHASE-05 以独立公共明文域实现单实例共享藏经阁，非默认分享密钥仅作上传维护凭据；先通过 GATE-03 再扩张 | 批准摘要 e88b58dea87dd1f409190b0b75759df08f7e9eafe8946763c77841124234096c + 基线 66f2ba500d8c11ddcdfbee4e1414b0c3bf92556a | 公共/私有事实源、明文边界、凭据语义、首期治理排除项、阶段或故障半径实质变化即失效；部署与远端 Git 仍另行授权 | 是 | APPROVED | DEC-16 |
