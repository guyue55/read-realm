...
> [!IMPORTANT]
> 本规格基于 2026-07-10 的代码、设计交接文档、生产构建记录与桌面/移动端活体页面。用户已授权在出现选择时自动采用推荐方案并持续推进，因此本文的推荐方案视为已批准。
...
| 方案 | 用户价值 | 成本与风险 | 结论 |
| --- | --- | --- | --- |
| A. 克制的中文阅读器 | 保留“墨问”辨识度，同时降低理解门槛和首屏负担 | 需要统一既有组件和文案 | **采用** |
| B. 强国风沉浸叙事 | 品牌感最强 | 术语、假数据和装饰状态维护成本高 | 不采用 |
| C. 中性效率工具 | 最轻、最容易维护 | 与现有品牌和设计交接断层 | 不采用 |

视觉原则是“安静但不单调”：暖白和炭黑负责大面积可读性，松柏绿负责主操作，冷青蓝负责同步与 AI，低饱和朱砂负责警告和阅读进度。页面不再由米黄与绿色单调覆盖。
...
- 桌面：窄侧栏提供书架、搜索、导入、笔记、设置；内容区使用稳定顶部栏。
- 手机：顶部只保留页面标题和当前页面主操作；底部导航包含书架、搜索、导入、笔记、设置。
- 导航、滚动记忆、在线状态和响应式布局只有一个权威实现。
- 图标按钮使用 `lucide-react`，无可见文字的控件提供可访问名称和 Tooltip。
...
1. 紧凑页面标题与“导入书籍”主操作。
2. 仅在存在进度时展示“继续阅读”；不存在进度时展示一句可操作引导。
...
- 四个切片全部实现并分别提交。
- 无硬编码用户统计、无自动预置污染、无重复 Shell 真相源。
