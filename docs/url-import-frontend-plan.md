...
**全部六个阶段（A-F）已按本计划执行并逐阶段提交**（分支 `main`）：

| 阶段 | 内容 | 状态 | 提交 |
|---|---|---|---|
| A | 前端 Readability 解析引擎 + 识别层 | ✅ 完成 | `957ae40` |
| B | L1 静态抓取网络桥 + 前端多级路由 | ✅ 完成 | `7f2f873` |
| C | L2 headless 渲染通道 | ✅ 完成 | `66160f1` |
| D | 后端解析停用 + 章节并发抓取 + 进度结构化 | ✅ 完成 | `fc41a45` |
| E | 阅读懒加载离线化 + 手动协助通道 | ✅ 完成 | `6716932` |
| F | 抓取档位设置 + 档位并发联动 + 全量回归 | ✅ 完成 | `4339929` |

**实施偏差说明（工程决策，均不改变数据契约与边界）**：
- 阶段 C 原计划 headless 渲染落在 `apps/worker`（独立进程）；实际落地为 **API 内嵌服务**（`headless-fetch.service.ts`，puppeteer-core 直连系统 Chrome），避免新增部署复杂度（worker 目录无 package.json）。worker 进程位保留为未来扩展，不影响能力与验收。
- 阶段 D 后端解析接口 `/imports/url/parse` 保留（供未来/兼容），但前端**不再调用**；前端统一走多级抓取 + 前端解析。
- 阶段 F 第三方通道以**独立开关偏好**实现（默认关闭），实际第三方 fetcher 实现位预留，未硬造。
