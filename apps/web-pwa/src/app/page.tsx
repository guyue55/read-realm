...
export default function Home() {
  ...
  const books = useLiveQuery(async () => {
    ...
    if (searchQuery) {
      ...
      filtered = allBooks.filter((b) => b.title.toLowerCase().includes(q));
    }
  ...
  const handleFileUpload = async (
    ...
    try {
      ...
      setStatus(
        strings.shelf.parseSuccess.replace(
          "{count}",
          parsedBook.chapters.length.toString(),
        ),
      );
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-12 bg-[#F8F8F5] text-[#2F2A24]">
      <div className="mb-8 flex w-full max-w-4xl items-center justify-between gap-4">
        <h1 className="text-3xl font-bold">{strings.shelf.title}</h1>
        <button
          onClick={() => (window.location.href = "/settings")}
          className="rounded-lg border border-[#3A2D22] bg-white px-4 py-2 text-sm font-semibold text-[#3A2D22] shadow-sm transition-colors hover:bg-[#E8E3DA]"
        >
      ...
      <div className="w-full max-w-4xl mb-8 flex gap-2">
        ...
        >
          {isGlobalSearching
            ? strings.shelf.searchingGlobal
            : strings.shelf.globalSearch}
        </button>
      </div>
      ...
      <div className="w-full max-w-4xl">
              ...
              >
                <div>
                  ...
                  <div className="flex gap-2 mb-4">
                    <span className="text-xs px-2 py-0.5 bg-[#E8E3DA] rounded uppercase">
                      {book.format}
                    </span>
        ...
        {(() => {
          ...
          const uniqueGlobalResults = globalResults.filter(
            (b) => !localBookIds.has(b.id),
          );
          ...
          return (
            <div className="mt-12 mb-8">
              <h2 className="text-xl font-bold mb-6 text-gray-600 border-t pt-8">
                {strings.shelf.globalResultsTitle} ({uniqueGlobalResults.length}
                )
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {uniqueGlobalResults.map((book) => (
                  <div
                    key={book.id}
                    className="bg-white p-6 rounded-lg shadow-sm border border-dashed border-gray-300 flex flex-col justify-between opacity-80 hover:opacity-100 transition-opacity"
                  >
                    <div>
                      <h3 className="font-bold text-lg mb-2 line-clamp-2">
                        {book.title}
                      ...
                      <div className="flex gap-2 mb-4">
                        <span className="text-xs px-2 py-0.5 bg-gray-100 rounded uppercase text-gray-500">
                          {strings.shelf.notInLibrary}
                        </span>
                        <span className="text-xs text-gray-500">
                          {book.format}
                        </span>
