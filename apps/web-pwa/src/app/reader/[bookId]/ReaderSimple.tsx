  ...
  return (
    ...
    >
          ...
          >
            ...
            {!isPagination && (
                <div className="mt-12 flex justify-between items-center border-t border-[rgba(80,65,45,0.12)] pt-8 relative z-10">
                    ...
                    <button onClick={handleNext} className="px-6 py-3 border border-[#678055] text-[#678055] font-bold rounded-full text-sm hover:bg-[rgba(103,128,85,0.04)] transition-colors">{strings.reader.nextChapter}</button>
                </div>
            )}
          </div>
        </div>
      ...
      <div>
          ...
          {/* TOC Drawer */}
          <div className={`fixed inset-y-0 left-0 w-[300px] max-w-[85vw] ${isDark ? 'bg-[#232323]' : 'bg-white'} z-50 shadow-xl transition-transform duration-300 ${activePanel === "toc" ? "translate-x-0" : "-translate-x-full"}`}>
          ...
          <div className={`fixed bottom-0 inset-x-0 sm:inset-x-auto sm:left-1/2 sm:-translate-x-1/2 sm:w-[400px] sm:bottom-8 ${isDark ? 'bg-[rgba(35,35,35,0.96)] shadow-2xl' : 'bg-[rgba(255,252,245,0.96)] sm:bg-white sm:shadow-2xl shadow-[0_-4px_20px_rgba(80,65,45,0.08)]'} z-50 px-6 pt-8 pb-[calc(2rem+env(safe-area-inset-bottom))] sm:pb-8 transition-transform duration-300 rounded-t-[24px] sm:rounded-[24px] ${activePanel === "progress" ? "translate-y-0 sm:scale-100" : "translate-y-full sm:scale-95 sm:opacity-0"}`}>
             <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setActivePanel(null)} className={`${isDark ? 'text-[#8F8F8F] hover:bg-white/10' : 'text-[#6F665B] hover:bg-gray-100'} p-1 rounded-full`}>✕</button>
             </div>
             ...
             <div className={`flex justify-between text-sm ${isDark ? 'text-[#8F8F8F]' : 'text-[#6F665B]'}`}>
               ...
               <span className="shrink-0">{(chapter?.index || 0) + 1} / {toc.length} 章</span>
             </div>
          </div>
      </div>
    </main>
