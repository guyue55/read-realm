...
export function ReaderDefault({ bookId }: { bookId: string }) {
  ...
  return (
    <main
      className="fixed inset-0 overflow-hidden transition-colors duration-300 xl:flex xl:items-center xl:justify-center xl:bg-[#F7F1E6]"
    >
      {/* 
      ...
      */}
      ...
      >
        ...
        <div 
            ...
            style={{ backgroundColor: overlay1, borderRight: `1px solid ${borderColor}` }}
        >
          <h1 className="text-xl font-bold mb-8 font-serif" style={{ color: brandColor }}>墨问</h1>
          <nav className="flex flex-col gap-6 items-center flex-1 w-full">
            <div onClick={() => router.push("/library")} className="flex flex-col items-center gap-1 w-16 py-2 rounded-lg cursor-pointer hover:bg-[rgba(80,65,45,0.04)]">
              <div className="w-2 h-2 rounded-full bg-[#678055] opacity-0 hover:opacity-100 transition-opacity"></div>
              <span className="text-[10px] font-medium" style={{ color: brandColor }}>书架</span>
            </div>
            <div onClick={() => router.push("/search")} className="flex flex-col items-center gap-1 w-16 py-2 rounded-lg cursor-pointer hover:bg-[rgba(80,65,45,0.04)]">
              <div className="w-2 h-2 rounded-full bg-[#678055] opacity-0 hover:opacity-100 transition-opacity"></div>
              <span className="text-[10px] font-medium" style={{ color: brandColor }}>发现</span>
            </div>
            <div onClick={() => router.push("/import")} className="flex flex-col items-center gap-1 w-16 py-2 rounded-lg cursor-pointer hover:bg-[rgba(80,65,45,0.04)]">
              <div className="w-2 h-2 rounded-full bg-[#678055] opacity-0 hover:opacity-100 transition-opacity"></div>
              <span className="text-[10px] font-medium" style={{ color: brandColor }}>导入</span>
            </div>
            <div onClick={() => setActivePanel("settings")} className="flex flex-col items-center gap-1 w-16 py-2 rounded-lg cursor-pointer bg-[#E7EDE0]">
              <div className="w-2 h-2 rounded-full bg-[#678055]"></div>
              <span className="text-[10px] font-medium" style={{ color: brandColor }}>设置</span>
            </div>
          </nav>
        </div>
        ...
        <div 
            ...
            style={{ backgroundColor: overlay2, borderRight: `1px solid ${borderColor}` }}
        >
          <TocDrawer
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-transparent relative">
          ...
          >
            ...
            >
              <h1 className="text-3xl font-bold mb-10 font-serif text-center" style={{ color: currentThemeColors.text }}>{chapter.title}</h1>
              <div
              ...
              {!isPagination && (
                  <div className="mt-16 flex justify-between items-center border-t border-[rgba(80,65,45,0.12)] pt-8 relative z-10">
                      <button onClick={handlePrev} className="px-6 py-3 bg-[rgba(80,65,45,0.04)] rounded-full text-sm hover:bg-[rgba(80,65,45,0.08)] transition-colors text-inherit">{strings.reader.prevChapter}</button>
        ...
        <div 
            ...
            style={{ backgroundColor: overlay1, borderLeft: `1px solid ${borderColor}` }}
        >
           <AIReaderPanel isAiLoading={isAiLoading} aiSummary={aiSummary} isMobileDrawer={false} isDark={isDark} />
        </div>
      </div>
      ...
      */}
      ...
      >
        ...
        <ReaderTopBar
            title={chapter.title} isVisible={showMenu} isDesktop={false} isDark={isDark}
            onBack={() => router.push("/library")}
        ...
        >
          ...
          >
            <h1 className="text-2xl font-bold mb-8 font-serif">{chapter.title}</h1>
            <div
              className="reader-content whitespace-pre-wrap break-words [&_p]:break-inside-avoid [&_p]:mb-4"
              dangerouslySetInnerHTML={{ __html: chapter.content }}
            />
        ...
        <ReaderBottomBar
            isVisible={showMenu} activePanel={activePanel} isDark={isDark}
        ...
        {/* TOC Drawer */}
        <div className={`fixed inset-y-0 left-0 w-4/5 max-w-sm bg-[var(--theme-bg)] z-50 shadow-xl transition-transform duration-300 ${activePanel === "toc" ? "translate-x-0" : "-translate-x-full"}`} style={{ backgroundColor: currentThemeColors.bg }}>
        ...
        {/* AI Drawer */}
        <div className={`fixed inset-y-0 right-0 w-[85%] max-w-md bg-[var(--theme-bg)] z-50 shadow-xl transition-transform duration-300 ${activePanel === "ai" ? "translate-x-0" : "translate-x-full"}`} style={{ backgroundColor: currentThemeColors.bg }}>
          <AIReaderPanel isAiLoading={isAiLoading} aiSummary={aiSummary} isMobileDrawer={true} isDark={isDark} onClose={() => setActivePanel(null)} />
        </div>

        {/* Settings Sheet */}
        <div className={`fixed bottom-0 inset-x-0 bg-transparent z-50 transition-transform duration-300 rounded-t-[24px] overflow-hidden ${activePanel === "settings" ? "translate-y-0" : "translate-y-full"}`}>
        ...
        {/* Progress Sheet */}
        <div className={`fixed bottom-0 inset-x-0 ${isDark ? 'bg-[rgba(35,35,35,0.96)] shadow-[0_-4px_20px_rgba(0,0,0,0.5)]' : 'bg-[rgba(255,252,245,0.96)] shadow-[0_-4px_20px_rgba(80,65,45,0.08)]'} z-50 px-6 pt-8 pb-[calc(2rem+env(safe-area-inset-bottom))] transition-transform duration-300 rounded-t-[24px] ${activePanel === "progress" ? "translate-y-0" : "translate-y-full"}`}>
            <div className="flex justify-between items-center mb-6">
              <h3 className={`font-bold ${isDark ? 'text-[#CFCFCF]' : 'text-[#2F2A24]'}`}>阅读进度</h3>
              <button onClick={() => setActivePanel(null)} className={`${isDark ? 'text-[#8F8F8F]' : 'text-[#6F665B]'} p-1`}>✕</button>
            </div>
            ...
            <div className={`flex justify-between text-sm ${isDark ? 'text-[#8F8F8F]' : 'text-[#6F665B]'}`}>
