  ...
  return (
    ...
    >
      ...
      <div className={`fixed top-0 inset-x-0 h-12 bg-white shadow-sm z-20 flex items-center px-4 transition-transform duration-200 ${showMenu ? 'translate-y-0' : '-translate-y-full'}`}>
        ...
        <button onClick={toggleAiPanel} className="ml-4 text-sm font-medium text-purple-600">{strings.reader.aiSummary}</button>
        <button onClick={addBookmark} className="ml-4 text-sm font-medium text-blue-600">{strings.reader.bookmark}</button>
      </div>

      {/* Settings Sheet */}
      <div className={`fixed bottom-0 inset-x-0 bg-white shadow-[0_-2px_10_rgba(0,0,0,0.1)] z-40 px-6 pt-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] transition-transform duration-300 ${showSettings ? 'translate-y-0' : 'translate-y-full'}`}>
      ...
      {/* Progress Sheet */}
      <div className={`fixed bottom-0 inset-x-0 bg-white shadow-[0_-2px_10_rgba(0,0,0,0.1)] z-40 px-6 pt-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] transition-transform duration-300 ${showProgress ? 'translate-y-0' : 'translate-y-full'}`}>
      ...
      <div className={`fixed bottom-0 inset-x-0 h-[calc(56px+env(safe-area-inset-bottom))] pb-[env(safe-area-inset-bottom)] bg-white shadow-[0_-1px_3px_rgba(0,0,0,0.1)] z-20 flex items-center justify-around px-4 transition-transform duration-200 pointer-events-auto ${showMenu ? 'translate-y-0' : 'translate-y-full'}`}>
        ...
        <button onClick={toggleProgress} className="text-sm active:scale-95 transition-transform">{strings.reader.progress}</button>
        <button onClick={toggleAiPanel} className="text-sm text-purple-600 font-bold active:scale-95 transition-transform">AI</button>
        <button onClick={toggleSettings} className={`text-sm active:scale-95 transition-transform ${showSettings ? 'text-blue-500 font-bold' : ''}`}>{strings.reader.settings}</button>
        <button onClick={handleNightModeToggle} className="text-sm active:scale-95 transition-transform">
