...
export default function NotesPage() {
  ...
  return (
    <PageLayout title="笔记" subtitle={`${notes.length} 条本地记录`} onBack={() => router.push("/library")}>
      <div className="mx-auto mt-4 w-full max-w-5xl space-y-5">
        {/* 筛选区：导出按钮与搜索/筛选在同一卡片，统一下边缘对齐 */}
        <section className="ui-card p-4 sm:p-5">
          <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_240px_auto]">
            <label className="relative">
              <span className="sr-only">搜索笔记</span>
              <Search aria-hidden="true" className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--color-quiet)]" size={18} />
              <input
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="搜索摘录、批注或书名"
                className="ui-focus-ring h-11 w-full rounded-[var(--radius-field)] border border-[var(--color-border)] bg-white/80 pl-10 pr-3 text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)]"
              />
            </label>
            <label>
              ...
              <select
                ...
                className="ui-focus-ring h-11 w-full rounded-[var(--radius-field)] border border-[var(--color-border)] bg-white/80 px-3 text-sm text-[var(--color-text)]"
              >
            ...
            <div className="flex flex-wrap gap-2 md:justify-end">
              <button
                ...
                className="ui-focus-ring inline-flex min-h-11 items-center justify-center rounded-[var(--radius-control)] border border-[var(--color-border)] bg-white/80 px-4 text-sm font-semibold text-[var(--color-text)] disabled:opacity-50"
              >
        ...
        {operationError && (
          <p
            role="alert"
            className="rounded-[var(--radius-control)] border border-[#E7B8AF] bg-[#FFF0EC] px-4 py-3 text-sm text-[var(--color-danger)]"
          >
            {operationError}
          </p>
        )}
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description={
              notes.length === 0
                ? "阅读时添加书签或批注，内容会保存在这台设备。"
                : "调整关键词或书籍筛选后再试。"
            }
            primaryAction={
              notes.length === 0
                ? { label: "去阅读", onClick: () => router.push("/library") }
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {filtered.map((note) => (
              <article key={note.id} className="ui-card flex flex-col rounded-[var(--radius-card)] p-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <h2 className="truncate font-bold text-[var(--color-text)]">
                      {note.bookTitle}
                    </h2>
                  ...
                  <button
                    ...
                    className="ui-focus-ring flex h-11 w-11 shrink-0 items-center justify-center rounded-[var(--radius-control)] text-[var(--color-danger)] hover:bg-[var(--color-danger-soft)]"
                  >
                    <Trash2 aria-hidden="true" size={18} />
                  </button>
                </div>
                <blockquote className="mt-4 flex-1 border-l-2 border-[var(--ui-warm)] pl-3 text-sm leading-6 text-[var(--color-muted)]">
                  {note.contentPreview || "无摘录预览"}
                </blockquote>
                {note.note && (
                  <p className="mt-4 rounded-[var(--radius-control)] bg-[var(--color-primary-soft)] p-3 text-sm leading-6 text-[var(--color-text)]">
                    {note.note}
                  </p>
                )}
                <div className="mt-4 flex items-center justify-between gap-2 border-t border-[var(--color-border)] pt-3 text-xs text-[var(--color-quiet)]">
