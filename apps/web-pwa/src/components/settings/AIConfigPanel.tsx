  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className={`text-sm font-bold ${textColor}`}>AI 配置</h3>
        <span
          className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${
            aiStatus.available
              ? aiStatus.source === "user"
                ? isDark
                  ? "bg-[#678055]/15 text-[#9DB98B]"
                  : "bg-[var(--color-primary-soft)] text-[var(--color-primary)]"
                : isDark
                  ? "bg-[#9A6A3A]/15 text-[#C9A57B]"
      ...
      <p className={`text-[10px] ${mutedText} leading-relaxed pt-1`}>
        你的 API 密钥使用 AES-GCM 加密后存储在浏览器本地， 仅在使用 AI
        功能时通过 HTTPS 加密传输。 你也可以在服务端通过环境变量 OPENAI_API_KEY
        配置全局密钥。
      </p>
    </div>
  );
